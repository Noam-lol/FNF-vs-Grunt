...
class PlayState extends MusicBeatState
{
	...
	var thefuckTxt:FlxText;
	var thefucktxtpeniscock:FlxPoint;
			...
			case 'caca':
				{
					...
					var bg:FlxSprite = new FlxSprite(-1061.5, -571.4).loadGraphic(Paths.image('cumbat/1/white'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.6, 0.6);
					bg.active = true;
					add(bg);

					var ground:FlxSprite = new FlxSprite(-911.9, -400).loadGraphic(Paths.image('cumbat/1/R nd g'));
					ground.antialiasing = true;
					ground.scrollFactor.set(0.9, 0.9);
					ground.active = false;
					add(ground);

					/*var bghouse:FlxSprite = new FlxSprite(-1061.5, -493.3).loadGraphic(Paths.image('cumbat/1/back'));
	...
	*/
	...
	override function stepHit()
	{
		...
		if (curStep == 628 && curSong == 'bobas' && thefuckTxt == null)
			{
				thefuckTxt = new FlxText(0, 475, 0, "THE FUCK ARE TALKING BOUT?!", 70);
				thefuckTxt.cameras = [camHUD];
				thefuckTxt.setFormat(Paths.font("impact.ttf"), 70, FlxColor.RED, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.RED);
				thefuckTxt.screenCenter(X);
				thefucktxtpeniscock = thefuckTxt.getPosition();
				add(thefuckTxt);
				...
				new FlxTimer().start(1, function(tmr:FlxTimer)
					{
						thefuckTxt.setPosition(10000,10000);
					});
			}
		
		if (curStep == 341 && curSong == 'bobas')
			{
				remove(thefuckTxt);
			}
		
		if (curSong == 'bobas' && curStep == 628 && thefuckTxt != null)
			{
				thefuckTxt.setPosition(FlxG.random.float(-15, 15) + thefucktxtpeniscock.x, FlxG.random.float(-15, 15) + thefucktxtpeniscock.y);
			}
